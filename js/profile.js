...
function getUserData(){
    ...
    axios.get(`https://tarmeezacademy.com/api/v1/users/${userId}`).then((response)=>{
        ...
        document.getElementById("profile-img").src=userData.profile_image.length?userData.profile_image:"./images/avatar-02.png"
        document.getElementById("user-input-name").value=userData.name
        document.getElementById("user-input-email").value=userData.email
        document.getElementById("user-input-username").value=userData.username
        document.getElementById("num-posts").innerText=userData.posts_count
        document.getElementById("num-comments").innerText=userData.comments_count
        showLoader(false)
    }).catch((error)=>{
        console.log(error)
        showLoader(false)
    ...
    axios.get(`https://tarmeezacademy.com/api/v1/users/${userId}/posts`).then((response)=>{
        ...
        for(post of allPosts){
            ...
            for(tage of post.tags){
                postTages +=`<button type="button" class="btn btn-secondary btn-sm rounded-pill">${tage.name}</button>`
            }
          ...
          icons=` <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" class="bi bi-three-dots primary-color" viewBox="0 0 16 16">
            ...
            <div class="post my-4 second-color">
                  <div class="d-flex gap-2 align-items-center p-3">
                    <img src="${post.author.profile_image.length?post.author.profile_image:"./images/avatar-02.png"}" alt="img" class="rounded-circle user-img">
                    <span class="lh-sm">
